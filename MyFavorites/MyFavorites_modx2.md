...
* Публичные ссылки на списки избранного — посторонний посетитель может просмотреть список по ссылке без авторизации.
...
### Генерация ключей публичных ссылок для существующих списков

В версии **1.1.0-pl** добавлена поддержка публичных ссылок на списки избранного (сниппет **MyFavorites.share**).

Если компонент был обновлён с версии **1.0.2-pl** или более ранней, у существующих списков избранного поле `share_key` останется пустым. В этом случае публичные ссылки для таких списков не будут работать.

Чтобы сгенерировать ключи публичных ссылок для всех существующих списков, выполните команду:

```bash
php полный_путь/core/components/myfavorites/scripts/generate_lists_share_key.php
```
...
### Публичные ссылки
В данном разделе вы можете настроить публичные ссылки на списки избранного (сниппеты **MyFavorites.lists** и **MyFavorites.share**):

* `myfavorites_share_key_var` — имя GET-параметра, содержащего ключ публичной ссылки.
* `myfavorites_share_key_length` — длина случайно генерируемого ключа для публичной ссылки на список избранного. По умолчанию: 16 символов (минимум — 8). Чем длиннее ключ, тем сложнее его подобрать.
* `myfavorites_share_page_id` — ID страницы, на которую будет вести публичная ссылка на список избранного. Если указано значение 0, ссылка будет вести на текущую страницу. На целевой странице должен быть вызван сниппет **MyFavorites.share**.
...
| element       | Только если `output=list`. Имя класса ресурса, из которого выбираются элементы списка (передаётся во вложенный `snippet`, например `pdoPage`). По умолчанию: pdoResources. |
...
| tplEmpty      | Только если `output=list`. Имя чанка, который будет выведен, если ссылка недействительна (ключ не найден, список отключён или пуст). По умолчанию: tpl.MyFavorites.empty. |

Если ссылка недействительна (ключ не найден, список отключён или пуст) и `output=list`, сниппет вместо вложенного `snippet` выводит чанк `tplEmpty` — в него передаётся переменная `share` со значением `true`, что позволяет отличить это сообщение от сообщения о пустом списке избранного (см. **MyFavorites** ниже). Чанк по умолчанию (`tpl.MyFavorites.empty`) в этом случае показывает `[[%myfavorites_err_share_not_found]]`.

Если `output=ids`, поведение при недействительной ссылке не меняется — сниппет возвращает `-0` (или пустой массив при `idsFormat=array`), и обработку сообщения об ошибке (например через `[[%myfavorites_err_share_not_found]]`, как в примере ниже, аналогично **MyFavorites.ids**) должен реализовать вызывающий код.

В режиме `output=list` в вызываемый `snippet` также передаются все остальные параметры, указанные при вызове **MyFavorites.share** (кроме `parents` и `resources`, которые сниппет всегда выставляет сам) — например `element`, `tpl`, `limit` для **pdoPage**.

**Пример вызова (HTML через pdoPage, по умолчанию)**
```html
[[!MyFavorites.share?
&element=`msProducts`
&limit=`12`
...
### MyFavorites - Вывод списка избранного пользователя

Получает ID элементов текущего списка избранного и выводит ресурсы через вложенный сниппет (аналогично связке **MyFavorites.ids** + `pdoPage`), опционально оборачивая результат в чанк с кнопками очистки списка и публичной ссылки.
...
|-------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| list        | Идентификатор списка. По умолчанию: default.                                                                                                                                                                        |
| snippet     | Имя сниппета, которому будут переданы ID ресурсов списка избранного (в параметре `resources`) для вывода. По умолчанию: pdoPage.                                                                                   |
| tplEmpty    | Имя чанка, который будет выведен, если список пуст. По умолчанию: tpl.MyFavorites.empty.                                                                                                                            |
| tplWrapper  | Имя чанка-обёртки для результата. Если не пусто, в чанк передаются плейсхолдеры `output` (результат вложенного `snippet`) и `list` (данные списка избранного, включая `share_url`). Если пусто — обёртка не применяется. По умолчанию: tpl.MyFavorites. |
| sharePageId | Только если указан `tplWrapper`. ID страницы, на которую будет вести публичная ссылка (`list.share_url`) в чанке-обёртке. Если не указан, используется системная настройка `myfavorites_share_page_id`. По умолчанию: 0. |

Если список пуст, сниппет возвращает чанк `tplEmpty` (в него передаётся переменная `share` со значением `false`) — этим он отличается от вызова **MyFavorites.share** для недействительной/пустой ссылки, где в тот же чанк передаётся `share` со значением `true` (см. **MyFavorites.share** выше).

В вложенный `snippet` передаются все остальные параметры, указанные при вызове **MyFavorites** (кроме `parents` и `resources`, которые сниппет всегда выставляет сам, и `tplWrapper`, который дальше не пробрасывается) — например `element`, `tpl`, `limit` для **pdoPage**.

Чанк по умолчанию (`tpl.MyFavorites`) выводит кнопку очистки списка (`data-myfavorites-clear`) и, если для списка доступна публичная ссылка (`list.share_url`), кнопку/ссылку `data-myfavorites-share` рядом с результатом `output`.
...
**Пример вызова с обёрткой (кнопки очистки списка и публичной ссылки "из коробки")**
```html
[[!MyFavorites?
&element=`msProducts`
&limit=`12`
&tplWrapper=`tpl.MyFavorites`
]]
```
